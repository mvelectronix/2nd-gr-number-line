document.addEventListener('DOMContentLoaded', () => {
    ...
    const MARKER_SIZE = 36; // keep in sync with CSS

    const toPercent = (value, min, max) => ((value - min) / (max - min)) * 100;
    const isWithin = (value, range) => value >= range.min && value <= range.max;

    const clampToVisibleArea = (percent, lineWidth) => {
        const edgePercent = Math.min(8, (MARKER_SIZE / lineWidth) * 50);
        return Math.min(100 - edgePercent, Math.max(edgePercent, percent));
    };
    ...
    function placeMarker(marker, line, value) {
        const width = line.element.clientWidth || 1;
        const safePercent = clampToVisibleArea(toPercent(value, line.min, line.max), width);
